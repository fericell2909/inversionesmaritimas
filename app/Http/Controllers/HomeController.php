...
class HomeController extends Controller
{
    ...
    public function index(){
	    ...
	    $salequantity=DB::table('sales')->select(DB::raw(' COUNT(*) as total, products.p_gname as money'))->join('products','products.p_id','=','sales.product_id')->groupBy(DB::raw('products.p_gname LIMIT 10'))->get();
	
	    //Sales money within a year
	    $salemoney=DB::table('sales')->select(DB::raw('SUM(sales.price) as money, MONTHNAME(sales.created_at) as month '))->groupBy(DB::raw('DATE_FORMAT(sales.created_at, "%m")'))->get();
	
	    //Amount of purchases within a year
	    $apurchases=DB::table('products')->select(DB::raw('SUM(p_imprice) as money, MONTHNAME(created_at) as month '))->whereNotIn('p_id',[1])->groupBy(DB::raw('DATE_FORMAT(created_at, "%m")'))->get();
	
	    // Total product
	    $totalproduct=DB::table('products')->select(DB::raw('count(p_id) as totalproduct '))->whereNotIn('p_id',[1])->get();
	    // Total sales
	    ...
	    $totalsale=null;
	    // Total purchases
	    $totalpurchases=DB::table('products')->select(DB::raw(' sum(p_imprice) as totalpurchases'))->whereNotIn('p_id',[1])->get();
	
	    // Last customers
	    $customers=DB::table('customers')->where('estado_id',1)->orderBy('id','desc')->limit(5)->get();
	
	    // Last notes
	    $note=DB::table('notes')->orderBy('id','desc')->limit(3)->get();
	    // Total product
	    $totalcustomers=DB::table('customers')->select(DB::raw('count(id) as customers'))->get();
	
	    $dayProduct=DB::table('products')->selectRaw(' count(p_id)  AS price, DAYNAME(created_at) AS day')->whereRaw('DATE(created_at) = DATE(NOW() - INTERVAL 1 DAY) OR DATE(created_at) =  DATE(NOW())')->groupBy('day')//->whereNotIn('p_id', [1])
		    ->orderBy('created_at')->get();
	
	    $daySales=DB::table('sales')->selectRaw(' SUM(sales.price * ( 100.0 - products.p_discount ) / 100.0) AS price, DAYNAME(sales.created_at) AS day')->join('products','products.p_id','=','sales.product_id')
            ->whereRaw('DATE(sales.created_at) = DATE(NOW() - INTERVAL 1 DAY) OR DATE(sales.created_at) =  DATE(NOW())')
            #->whereRaw('DATE(sales.created_at) = DATE(NOW())')
            ->groupBy('day')
        ...
        if (!isset($daySales[0]->price) && !isset($daySales[1]->price)) {
            ...
            $daySales = array('yasterday' => '0' , 'today' => $daySales[0]->price);
